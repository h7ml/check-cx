...
export async function checkGemini(
  ...
  try {
    ...
    if (!collectedResponse || collectedResponse.trim() === "") {
      ...
      console.log(`[Gemini] ${config.groupName || "默认"} | ${config.name} | Q: ${challenge.prompt} | A: (空) | 期望: ${challenge.expectedAnswer} | 验证: 失败(空回复)`);
      return {
        id: config.id,
        name: config.name,
        type: config.type,
    ...
    // 打印对话日志
    console.log(`[Gemini] ${config.groupName || "默认"} | ${config.name} | Q: ${challenge.prompt} | A: ${collectedResponse} | 期望: ${challenge.expectedAnswer} | 验证: ${validationResult.valid ? "通过" : "失败"}`);
