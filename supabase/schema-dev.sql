...
COMMENT ON TABLE dev.check_models IS '模型配置表 - 存储可复用模型定义与模型级默认参数';
...
COMMENT ON COLUMN dev.check_models.request_header IS '模型默认请求头，JSONB 格式';
COMMENT ON COLUMN dev.check_models.metadata IS '模型默认 metadata，JSONB 格式';
